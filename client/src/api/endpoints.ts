...
// Quizzes (core) endpoints
export const quizzesAPI = {
  getQuizzes: async (params?: Record<string, string | number | boolean>) => {
    const query = new URLSearchParams()
    if (params) {
      Object.entries(params).forEach(([k, v]) => query.append(k, String(v)))
    }
    const response = await axios.get(`/api/quizzes${query.toString() ? `?${query.toString()}` : ''}`)
    return response.data
  },
  ...
  createAttempt: async (quizId: string) => {
    const response = await axios.post(`/api/quizzes/${quizId}/attempts`)
    return response.data
  },

  submitAttempt: async (quizId: string, attemptId: string, payload: any) => {
    const response = await axios.post(`/api/quizzes/${quizId}/attempts/${attemptId}/submit`, payload)
    return response.data
  },
...
// Tutorials endpoints (Tutorial Hub)
export const tutorialsAPI = {
  getTutorials: async (params?: Record<string, string | number | boolean>) => {
    const query = new URLSearchParams()
    if (params) Object.entries(params).forEach(([k, v]) => query.append(k, String(v)))
    const response = await axios.get(`/api/tutorials${query.toString() ? `?${query.toString()}` : ''}`)
    return response.data
  },
...
// Rack (MyRack) notes endpoints — separate from quick notes if needed
export const rackAPI = {
  getNotes: async (page = 1, limit = 9) => {
