...
const StudyPESSubjectViewer: React.FC = () => {
  ...
  const handleFileOpen = (material: StudyPESMaterial) => {
    ...
    const extension = getFileExtension(material.fileName || '');
    const fileUrl = `http://localhost:5050/api/files/${material.gridFSFileId}`;
  ...
  const handleFileDownload = (material: StudyPESMaterial) => {
    ...
    link.href = `http://localhost:5050/api/files/${material.gridFSFileId}/download`;
                    ...
                    <button
                      onClick={() => {
                        if (selectedMaterial.gridFSFileId) {
                          window.open(`http://localhost:5050/api/files/${selectedMaterial.gridFSFileId}`, '_blank');
                        } else {
                          alert('File not available for download');
                        }
                      }}
              ...
              {selectedMaterial.gridFSFileId ? (
                <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.4.120/build/pdf.worker.min.js">
                  <div style={{ height: '100%', width: '100%' }} className="relative">
                    <Viewer
                      fileUrl={`http://localhost:5050/api/files/${selectedMaterial.gridFSFileId}`}
