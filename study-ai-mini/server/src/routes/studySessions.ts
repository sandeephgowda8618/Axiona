...
import { StudySession, DailyPlan, Streak, PerformanceInsight } from '../models'
...
async function updateUserStreak(userId: any, studyDate: Date) {
  try {
...
async function generatePerformanceInsight(userId: any, session: any) {
  try {
    ...
    const insights = [
      ...
      },
      {
        ...
        summary: `Your ${session.pulseIntervals.length} pulse intervals show active engagement.`,
