...
interface IQuestion extends Document {
  id: string;
  question: string;
  type: 'multiple-choice' | 'single-choice' | 'true-false' | 'numerical' | 'essay';
  options?: string[];
...
QuizSchema.virtual('averageDifficulty').get(function(this: IQuiz) {
...
// Methods
QuizSchema.methods.addQuestion = function(questionData: Partial<IQuestion>) {
  this.questions.push(questionData);
  this.totalQuestions = this.questions.length;
  this.maxMarks = this.questions.reduce((sum: number, q: IQuestion) => sum + q.marks, 0);
  return this.save();
};

QuizSchema.methods.removeQuestion = function(questionId: string) {
  this.questions = this.questions.filter((q: IQuestion) => q.id !== questionId);
  this.totalQuestions = this.questions.length;
  this.maxMarks = this.questions.reduce((sum: number, q: IQuestion) => sum + q.marks, 0);
  return this.save();
};
...
QuizSchema.methods.getActiveQuestions = function() {
  return this.questions.filter((q: IQuestion) => q.marks > 0);
};
