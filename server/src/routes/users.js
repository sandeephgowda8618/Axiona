...
router.post('/sync-firebase', async (req, res) => {
  try {
    const { firebaseUid, email, fullName, avatarUrl } = req.body;
    ...
    if (user) {
      ...
      // Check if user exists with this email (migrating existing user)
...
router.get('/stats/:userId', async (req, res) => {
  try {
    ...
    const highlightsCount = await Highlight.countDocuments({ userId });

    // TODO: Add more stats as needed
    const stats = {
      totalNotes: notesCount,
      totalHighlights: highlightsCount,
      coursesCompleted: 0, // Implement when course system is ready
      streakDays: 0, // Implement streak calculation
      weeklyActivity: '0h' // Implement activity tracking
    };
